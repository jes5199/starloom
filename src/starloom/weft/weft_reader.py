...
from lib.horizons.quantities import EphemerisQuantity, OrbitalElementsQuantity
...
class WeftReader:
    ...
    def load_file(self, file_path: str, file_id: str, quantity: Optional[EphemerisQuantity | OrbitalElementsQuantity] = None) -> None:
        """
    ...
    def get_value(self, dt: datetime, file_id: str = None) -> float:
        ...
        if file_id is None:
            if len(self.files) != 1:
                raise ValueError("file_id must be provided when multiple files are loaded")
    ...
    def _interpolate_daily_blocks(self, blocks: List[DailyDataBlock], dt: datetime, file_id: str) -> float:
        """
        ...
        for block in blocks:
            ...
            midpoint = datetime(block.year, block.month, block.day, 0, 0, 0, tzinfo=timezone.utc)
        ...
        for i, block in enumerate(blocks):
            ...
            if -1 <= block_x_values[i] <= 1:
                ...
                return self._interpolate_remaining_blocks(blocks, block_values, block_x_values, dt, file_id)
    ...
    def _interpolate_remaining_blocks(self, blocks, block_values, block_x_values, dt, file_id):
    ...
    def _interpolate_fallback(self, blocks, dt, file_id):
        ...
        midpoints = [datetime(b.year, b.month, b.day, 0, 0, 0, tzinfo=timezone.utc) for b in blocks]
        closest_idx = min(range(len(blocks)), key=lambda i: abs((midpoints[i] - dt).total_seconds()))
    ...
    def get_info(self, key: str) -> Dict[str, Any]:
        ...
        # Count block types
        multi_year_count = sum(1 for block in weft_file.blocks if isinstance(block, MultiYearBlock))
        monthly_count = sum(1 for block in weft_file.blocks if isinstance(block, MonthlyBlock))
        daily_count = sum(1 for block in weft_file.blocks if isinstance(block, DailyDataBlock))
