...
from lib.weft.weft import (
    ...
    WeftFile, unwrap_angles
)
from lib.horizons.quantities import EphemerisQuantity, OrbitalElementsQuantity, ANGLE_QUANTITIES
from lib.horizons.planet import Planet
...
class WeftGenerator:
    ...
    def __init__(self, quantity: EphemerisQuantity):
        ...
        self.wrapping_behavior = 'wrapping' if quantity in [
    ...
    def _generate_samples(
        ...
        if self.wrapping_behavior == 'wrapping':
            ...
            if jumps:
                ...
                for i, (idx1, idx2, val1, val2, diff) in enumerate(jumps[:5]):  # Print first 5 jumps
                    print(f"  Jump {i+1}: Between indices {idx1}-{idx2}, values {val1:.2f}° -> {val2:.2f}°, diff: {diff:.2f}°")
                if len(jumps) > 5:
            ...
            if jumps:
                print(f"WARNING: Found {len(jumps)} large jumps (>180°) in unwrapped data:")
                for i, (idx1, idx2, val1, val2, diff) in enumerate(jumps[:5]):  # Print first 5 jumps
                    print(f"  Jump {i+1}: Between indices {idx1}-{idx2}, values {val1:.2f}° -> {val2:.2f}°, diff: {diff:.2f}°")
                if len(jumps) > 5:
                    print(f"  ... and {len(jumps) - 5} more jumps")
        ...
        if len(normalized_times) > 1:
            diffs = [normalized_times[i] - normalized_times[i-1] for i in range(1, len(normalized_times))]
            avg_diff = sum(diffs) / len(diffs)
            max_diff = max(diffs)
            min_diff = min(diffs)
            
            print(f"x_values distribution: avg diff = {avg_diff:.6f}, min diff = {min_diff:.6f}, max diff = {max_diff:.6f}")
            
            if max_diff > 2 * avg_diff:
                print(f"WARNING: x_values may not be evenly distributed. Max diff is {max_diff/avg_diff:.2f}x the average")
    ...
    def create_multi_year_block(
        ...
        return MultiYearBlock(start_year=start_year, duration=duration, coeffs=coeffs.tolist())
    ...
    def create_multi_precision_file(
        ...
        if config.get("yearly", {}).get("enabled", False):
            ...
            years = yearly_config.get("years", range(start_date.year, end_date.year + 1))
        ...
        if config.get("monthly", {}).get("enabled", False):
            ...
            years = monthly_config.get("years", range(start_date.year, end_date.year + 1))
