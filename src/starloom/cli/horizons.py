...
from ..horizons.time_spec import TimeSpec, TimeSpecType
...
@click.option('--date', '-d', multiple=True, type=click.DateTime(),
              help='Specific date(s) to request data for (UTC). Can be specified multiple times.')
@click.option('--start', '-s', type=click.DateTime(),
              help='Start date for date range (UTC).')
@click.option('--end', '-e', type=click.DateTime(),
              help='End date for date range (UTC).')
@click.option('--step', type=str,
              help='Step size for date range (e.g., "1h", "1d").')
def ecliptic(planet: str, date: List[datetime], start: Optional[datetime],
            end: Optional[datetime], step: Optional[str]):
    ...
    if date:
        ...
        dates = [d.replace(tzinfo=timezone.utc) if d.tzinfo is None else d for d in date]
        ...
        raise click.UsageError("Must specify either --date or all of --start, --end, and --step")
