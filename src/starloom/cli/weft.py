...
@click.argument("quantity", required=True, type=click.Choice([q.name for q in EphemerisQuantity]))
@click.option("--start-date", "-s", help="Start date (YYYY-MM-DD or Julian date)", required=True)
@click.option("--end-date", "-e", help="End date (YYYY-MM-DD or Julian date)", required=True)
...
@click.option("--prefetch-step", help="Step in hours for prefetching", default=24, type=int)
def generate(planet: str, quantity: str, start_date: str, end_date: str, output: str, 
             data_dir: str, prefetch: bool, prefetch_step: int) -> None:
