...
class RetrogradeFinder:
    ...
    def _get_position_at_time(self, jd: float) -> Optional[float]:
        ...
        try:
            ...
            positions = self.planet_ephemeris.get_planet_positions(
        ...
        for i in range(1, len(dates)):
            ...
            if find_velocity_crossing:
                ...
                if prev_diff * curr_diff <= 0:  # Sign change indicates crossing
                    ...
                    segments.append(
                        (prev_jd, curr_jd, prev_lon, curr_lon, prev_diff, curr_diff)
                    )
        ...
        if find_velocity_crossing:
            ...
            def compute_velocity(jd):
                ...
                try:
                    ...
                    time_spec = TimeSpec.from_dates(
                        [jd - time_delta, jd, jd + time_delta]
                    )
            ...
            while (right_jd - left_jd) > precision_days and iterations < max_iterations:
                ...
                if (find_pos_to_neg and mid_vel > 0) or (
                    not find_pos_to_neg and mid_vel < 0
                ):
                    left_jd = mid_jd
                else:
                    right_jd = mid_jd
            ...
            cross_lon = (
                ...
                else self._get_position_at_time(best_jd)
            )
            ...
            if cross_lon is None:
                try:
                    ...
                    fraction = (cross_jd - segments[0][0]) / (
    ...
    def _calculate_exact_angular_rate(
        self, jd: float, planet_name: str, is_sun: bool = False
    ) -> float:
    ...
    def _find_sun_aspect(
        ...
        try:
            ...
            precision_days = precision_seconds / (
                24.0 * 60.0 * 60.0
            ...
            def compute_difference(jd):
                ...
                try:
                    ...
                    sun_pos = self.sun_ephemeris.get_planet_positions("SUN", time_spec)[
                        jd
            ...
            while (right_jd - left_jd) > precision_days and iterations < max_iterations:
                ...
                if iterations % 3 == 0:  # Apply this every few iterations
                    try:
                        ...
                        planet_rate = self._calculate_exact_angular_rate(
                            mid_jd, planet.name
                        )
                        sun_rate = self._calculate_exact_angular_rate(
                            mid_jd, "SUN", is_sun=True
                        )
                        ...
                        if (is_inner and relative_rate < 0) or (
                            not is_inner and relative_rate > 0
                        ):
    ...
    def find_retrograde_periods(
        ...
        if coarse_dates and coarse_velocities[coarse_dates[0]] < 0:
            ...
            current_period = {
                "start_jd": first_jd
                - 60,  # Look back 60 days to find retrograde station
        ...
        for i in range(1, len(coarse_dates)):
            ...
            if prev_vel > 0 and curr_vel < 0:
                current_period = {
                    "start_jd": prev_jd
                    - 30,  # Look back 30 days to ensure we capture shadow start
        ...
        for period in potential_periods:
            ...
            station_retrograde = self._find_zero_crossing(
                ...
                fine_positions,
                fine_velocities,
                find_velocity_crossing=True,
                find_pos_to_neg=True,
            ...
            if not pre_shadow_start:
                ...
                try:
                    ext_time_spec = TimeSpec.from_range(
                        extended_start, julian_to_datetime(station_retro_jd), fine_step
                    )
            ...
            if not post_shadow_end:
                ...
                try:
                    ext_time_spec = TimeSpec.from_range(
                        julian_to_datetime(station_direct_jd), extended_end, fine_step
                    )
                    ext_positions = self.planet_ephemeris.get_planet_positions(
                        planet.name, ext_time_spec
                    )
            ...
            if planet != Planet.SUN:
                ...
                try:
                    ...
                    aspect_time_spec = TimeSpec.from_range(
                        aspect_start, aspect_end, "1h"
                    )
