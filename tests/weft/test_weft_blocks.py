...
from lib.weft.weft import (
    ...
    WeftFile, evaluate_chebyshev, unwrap_angles
)
...
class TestDailyBlocks(unittest.TestCase):
    def setUp(self):
        ...
        self.block_size = 22  # 2(marker) + 2(year) + 1(month) + 1(day) + 4*3(coeffs) + 4(padding)
...
class TestWeftFile(unittest.TestCase):
    def setUp(self):
        ...
        self.multi_year = MultiYearBlock(start_year=2000, duration=5, coeffs=[1.0, 0.5, -0.2])
        ...
        self.daily_header = DailySectionHeader(
            start_year=2003, start_month=1, start_day=1,
            end_year=2003, end_month=1, end_day=2,
            block_size=block_size, block_count=2
        )
        self.daily1 = DailyDataBlock(year=2003, month=1, day=1, coeffs=[0.3, 0.4, 0.5], block_size=block_size)
        self.daily2 = DailyDataBlock(year=2003, month=1, day=2, coeffs=[0.6, 0.7, 0.8], block_size=block_size)
        
        self.blocks = [self.multi_year, self.monthly, self.daily_header, self.daily1, self.daily2]
